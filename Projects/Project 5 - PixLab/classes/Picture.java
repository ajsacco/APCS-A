...
public class Picture extends SimplePicture 
{
  ...
  public void grayscale()
  {
    ...
    for (Pixel[] rowArray : pixels)
    {
      for (Pixel pixelObj : rowArray)
      {
        int level = (pixelObj.getBlue() + pixelObj.getBlue() + pixelObj.getBlue())/3; 
        pixelObj.setRed(level);
        pixelObj.setGreen(level);
        pixelObj.setBlue(level);
      }
    }
  }
    ...
    for (Pixel[] rowArray : pixels)
    {
      for (Pixel pixelObj : rowArray)
      {
        if(pixelObj.getRed() >= 17){
            pixelObj.setBlue((int)((double)(pixelObj.getBlue())*0.75));
        }else{
            pixelObj.setBlue((int)((double)(pixelObj.getBlue())*1.25));
  ...
  public void copyPart(Picture fromPic, 
                 int pasteRow, int pasteCol, int startRow, int startCol, int endRow, int endCol)
  {
    Pixel fromPixel = null;
    Pixel toPixel = null;
    Pixel[][] toPixels = this.getPixels2D();
    Pixel[][] fromPixels = fromPic.getPixels2D();
    System.out.println(pasteCol);
    for (int fromRow = startRow, toRow = pasteRow; 
         fromRow < endRow &&
         toRow < toPixels.length; 
         fromRow++, toRow++)
    {
      for (int fromCol = startCol, toCol = pasteCol; 
           fromCol < endCol &&
           toCol < toPixels[toRow].length;  
           fromCol++, toCol++)
      {
        fromPixel = fromPixels[fromRow][fromCol];
        toPixel = toPixels[toRow][toCol];
